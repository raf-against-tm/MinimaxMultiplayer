...
;ESTE CODIGO CORRESPONDE A LA VERSION 3 DE MINIMAX-MULTIPLAYER, EN LA QUE SE ESTABLECE LA POSIBILIDAD DE UNA PROFUNDIDAD MAXIMA PARA EL ARBOL DEL JUEGO.

(defun decision-maxn (actual profundidad) ;Para omitir la profundidad basta con pasar un valor negativo.
  ...
  (let ((max-val *minimo-valor*) (max-nodo ()) (jugador (turno actual)))
    (loop for nodo in (sucesores actual)
       do (let* ((puntuacion (valor-maxn nodo max-val (1- profundidad))))
            (if (>= (puntuacion-jugador puntuacion jugador) max-val)
                (progn (setf max-val (puntuacion-jugador puntuacion jugador))
                       (setf max-nodo nodo))))
       if (= max-val *maximo-valor*) do (loop-finish))
    max-nodo)
)

(defun valor-maxn (nodo cota-puntos profundidad)
...
(defun maximiza-puntuacion (sucesores jugador cota-puntos profundidad)
  ...
  (let ((max-val *minimo-valor*) (max-puntuacion ()))
        (loop for nodo in sucesores
           do (let* ((puntuacion (valor-maxn nodo max-val profundidad)))
            (if (>= (puntuacion-jugador puntuacion jugador) max-val)
                (progn (setf max-val (puntuacion-jugador puntuacion jugador))
                       (setf max-puntuacion puntuacion))))
           if (<= (- *maximo-valor* max-val) cota-puntos) do (loop-finish))
   max-puntuacion)
)
